...
DECLARE @SQL_QUERY NVARCHAR(max)='-- Set @N to the number of queries to capture by each ordering metric.  Since there are seven different metrics, 
...
-- Step 1: Capture a snapshot of (cumulative) query stats and store it in ##TEMP_KAR1 
IF OBJECT_ID (''tempdb..##TEMP_KAR1'') IS NOT NULL
BEGIN
    DROP TABLE ##TEMP_KAR1;
END;

IF OBJECT_ID (''tempdb..##TEMP_KAR1'') IS NOT NULL
BEGIN
    TRUNCATE TABLE ##TEMP_KAR1;

	/* CREATE TABLE [dbo].[##TMP_KAR2](
	[collection_time] [datetime] NOT NULL,
	[query_fingerprint] [binary](8) NULL,
	[plan_fingerprint] [binary](8) NULL,
	[plan_count] [int] NULL,
	...
	[plan_count_rank] [bigint] NULL,
	[cpu_rank] [bigint] NULL,
	[physical_reads_rank] [bigint] NULL,
	[logical_reads_rank] [bigint] NULL,
	[logical_writes_rank] [bigint] NULL,
	[max_duration_rank] [bigint] NULL,
	[execution_count_rank] [bigint] NULL,
...
-- Step 2: Rank the plan fingerprints by CPU use, execution count, etc and store the results in ##TMP_KAR2 
...
(
    SELECT 
        ...
        cur_stats.dbname
    FROM ##TMP_KAR2 AS cur_stats
    LEFT OUTER JOIN ##TMP_KAR2 AS prev_stats
