DECLARE @CLIENTIP VARCHAR(100)=NULL--'10.20.114.196'--'10.20.101.83',10.20.100.150,10.20.114.195
,@SESSION VARCHAR(20)=NULL
,@CONDITION VARCHAR(20)='SESS'--SESS,TABLOAD,SPACE

IF ISNULL(@CONDITION,'')='SESS'
BEGIN
...
SELECT --isnull(clientnetaddress,[host_name])Machine,
S.[database_name]
--,replace([login_name],'NT AUTHORITY\SYSTEM','SYSTEM')[login_name]
,CASE WHEN login_name='UJJIVAN\UJJ42570' THEN 'UJJ42570-LAVANYA(193-OS_L1)'
WHEN login_name='UJJIVAN\UJJ31404' THEN 'UJJ31404-SACHIN(193-OS_L2)'
WHEN login_name='UJJIVAN\UJJ44607' THEN 'UJJ44607-PRUDHVI(194-OS_L1)'
WHEN login_name='UJJIVAN\UJJ42568' THEN 'UJJ42568-Rashmi(194-OS_L2)'
WHEN login_name='UJJIVAN\UJJ25591' THEN 'UJJ25591-SWAPNIL(195-OS_L1)'
WHEN login_name='UJJIVAN\UJJ29457' THEN 'UJJ29457-RAVI(195-OS_L2)'
WHEN login_name='UJJIVAN\MU0168' THEN 'MU0168-Sanghavi(196-OS_L1)'
WHEN login_name='UJJIVAN\UJJ42437' THEN 'UJJ42437-Amlan(196-OS_L2)'
WHEN login_name='UJJIVAN\UJJ42569' THEN 'UJJ42569-KARUNAKAR(197-OS_L2)'
WHEN login_name='UJJIVAN\MU0166' THEN 'MU0166-SAI(197-OS_L1)'
WHEN login_name='UJJIVAN\MU0169' THEN 'MU0169-Rahul(198-OS_L1))'
WHEN login_name='UJJIVAN\UJJ42188' THEN 'UJJ42188-SINDHU(199-OS_L1)'
WHEN login_name='UJJIVAN\UJJ44666' THEN 'UJJ44666-Sourabh(199-OS_L2)'
WHEN login_name='UJJIVAN\MU0161' THEN 'MU0161-GOPI(200-OS_L1)'
WHEN login_name='UJJIVAN\MU0170' THEN 'MU0170-Usha(201-OS_L1)'
WHEN login_name='NT AUTHORITY\SYSTEM' THEN 'SYSTEM(74)' 
WHEN login_name='UJJIVAN\sqlclusadmin' or login_name='UJJIVAN\SQLSERVICEADMIN' THEN 'SQL-ADMIN' 
WHEN login_name='EXTUSERS' THEN 'ETL(150)' 
WHEN login_name='ReportUser' AND clientnetaddress='10.20.100.150' THEN 'ETL(150)' 
WHEN login_name='ReportUser' AND clientnetaddress<>'10.20.100.150' THEN 'Reportserver(83)' 
ELSE CONCAT(isnull(clientnetaddress,[host_name]),'--',login_name) END 'LOGIN_NAME'
,concat(ltrim(s.[session_id])
,case when Sessions_sub_count>0 then concat(replicate('_',3),Sessions_sub_count) end
,case when c.blocking_session_id>0 then concat(replicate('_',3),c.blocking_session_id) end)
'sessions-Blocked'
--,c.blocking_session_id--replace(c.blocking_session_id,0,null)blocking_session_id
...
,CASE WHEN elapsed_time < 0 THEN RIGHT ( REPLICATE('0', max_elapsed_length) + 
	...
	, ltrim(CONVERT(VARCHAR,SPACE(MAX(LEN(CONVERT(VARCHAR, CPU))) OVER() - LEN(CONVERT(VARCHAR, CPU))) + LEFT(CONVERT(CHAR(22), 
	CONVERT(MONEY, CPU), 1), 19))) AS CPU
	 
	 ,[sql_text],procedurename,[wait_info]
	 ,command,[login_time],start_time
 ...
 --,GETDATE() AS [collection_time] 
	...
	--, ltrim(CONVERT(VARCHAR,SPACE(MAX(LEN(CONVERT(VARCHAR, tempdb_current))) OVER() - LEN(CONVERT(VARCHAR, tempdb_current))) + 
	--LEFT(CONVERT(CHAR(22), CONVERT(MONEY, tempdb_current), 1), 19))) AS tempdb_current

	--,ltrim(CONVERT(VARCHAR, SPACE(MAX(LEN(CONVERT(VARCHAR, used_memory))) OVER() - LEN(CONVERT(VARCHAR, used_memory))) + 
	...
	--,CONVERT(VARCHAR,SPACE(MAX(LEN(CONVERT(VARCHAR, blocked_session_count))) OVER()-LEN(CONVERT(VARCHAR, blocked_session_count))) + 
	 ...
	 --,row_count,[program_name]
	 ---------------------------------------------to know the backup status start---------------
	 /*
	 ,[name] 'BACKUP_DATABASE'
	 ,CASE WHEN name IS NOT NULL THEN [% Complete] else null end [% Complete]
	 ,CASE WHEN name IS NOT NULL THEN [Estimated Completion Time] else null end [Estimated Completion Time]
	 ,CASE WHEN name IS NOT NULL THEN [Elapsed Min] else null end [Elapsed Min]
	 ,CASE WHEN name IS NOT NULL THEN [Estimated Min] else null end [Estimated Min]
	 ,CASE WHEN name IS NOT NULL THEN [Estimated Hours] else null end [Estimated Hours]	 
	 ,[last_backup_START_time]
	 ,Last_BackUp_END_Time,Backup_Duration,recovery_model_desc,state_desc,backup_type,physical_device_name,backup_size,server_name,compressed_backup_size
	*/
	 ---------------------------------------------to know the backup status end---------------
FROM #TMP_SESSIONS_KAR s 
inner join (SELECT distinct session_id
	  ,CASE MAX ( LEN ( CONVERT ( VARCHAR, CASE WHEN elapsed_time < 0 THEN (-1 * elapsed_time)
			...
			END AS max_elapsed_length 
FROM #TMP_SESSIONS_KAR AS s1 
			WHERE s1.recursion = 1 and session_id<>'51') b on s.session_id=b.session_id
left join (
select distinct 
...
--((case req.statement_end_offset when -1 then datalength(sqltext.text) else req.statement_end_offset 
--end - req.statement_start_offset)/2)+1) as query
--,CASE WHEN sqltext.text LIKE 'BACKUP%Differential Backups%' THEN 'Differential database' 
--WHEN sqltext.text LIKE 'BACKUP%Backups%' THEN sqltext.text
--END 'backup_type'
...
,CONVERT(NUMERIC(6, 2), req.percent_complete) AS [% Complete]
    ...
    --,last_execution_time
		 ---------------------------------------------to know the backup status start---------------
		 /*
	,D.name,ISNULL(CONVERT(VARCHAR,backup_start_date),backup_start_date) AS last_backup_START_time
,ISNULL(CONVERT(VARCHAR,backup_finish_date),backup_finish_date)  AS Last_BackUp_END_Time
,STUFF(CONVERT(VARCHAR(20),backup_finish_date-backup_start_date,114),1,2,
DATEDIFF(hh,0,backup_finish_date-backup_start_date)) Backup_Duration
,D.recovery_model_desc,state_desc,CASE WHEN type ='D' THEN 'Full database'
WHEN type ='I' THEN 'Differential database'
WHEN type ='L' THEN 'Log'
WHEN type ='F' THEN 'File or filegroup'
WHEN type ='G' THEN 'Differential file'
WHEN type ='P' THEN 'Partial'
WHEN type ='Q' THEN 'Differential partial'ELSE type END AS backup_type
,physical_device_name,backup_size,server_name,compressed_backup_size
*/
	 ---------------------------------------------to know the backup status end---------------
...
LEFT join (select distinct spid,case when blocked>0 then blocked else null end blocked from sys.sysprocesses
where case when blocked>0 then blocked else null end is not null) sys on req.session_id=sys.spid
LEFT join (select spid,count(1)Sessions_sub_count from sys.sysprocesses
where spid>50 group by spid) sys1 on req.session_id=sys1.spid
	 ---------------------------------------------to know the backup status start---------------
	 /*
left join sys.databases D on DB_NAME(req.database_id)=D.name AND command='BACKUP DATABASE'
LEFT JOIN (SELECT database_name,backup_start_date,type,physical_device_name,backup_finish_date
,Row_Number() OVER(PARTITION BY database_name,BS.type ORDER BY backup_start_date DESC) AS RowNum,backup_size
,server_name,compressed_backup_size FROM msdb..backupset BS JOIN msdb.dbo.backupmediafamily BMF 
ON BS.media_set_id=BMF.media_set_id) CTE ON D.name = CTE.database_name AND RowNum = 1
and  CASE WHEN sqltext.text LIKE 'BACKUP%Differential Backups%' THEN 'Differential database' 
WHEN sqltext.text LIKE '%Backups%' THEN sqltext.text
END=CASE WHEN type ='D' THEN 'Full database'
WHEN type ='I' THEN 'Differential database'
WHEN type ='L' THEN 'Log'
WHEN type ='F' THEN 'File or filegroup'
WHEN type ='G' THEN 'Differential file'
WHEN type ='P' THEN 'Partial'
WHEN type ='Q' THEN 'Differential partial'ELSE type END 
*/
	 ---------------------------------------------to know the backup status start---------------
--order by req.session_id desc
			...
			where s.recursion = 1 
			AND clientnetaddress=ISNULL(@CLIENTIP,clientnetaddress)
			--AND s.SESSION_ID=ISNULL(@SESSION,s.SESSION_ID)
			AND (s.SESSION_ID=ISNULL(@SESSION,s.SESSION_ID) OR C.blocking_session_id=ISNULL(@SESSION,s.blocking_session_id))
			ORDER BY s.[start_time] ASC		
		END;
END

ELSE IF ISNULL(@CONDITION,'')='TABLOAD'
BEGIN
SELECT TaskName,*
FROM MISREPORT_DB.intdata.Audit_ETLPkg(NOLOCK) B where convert(date,LoadEndTime)=convert(date,getdate()-3) 
and TaskName not in (SELECT TaskName
FROM MISREPORT_DB.intdata.Audit_ETLPkg(NOLOCK) B where convert(date,LoadEndTime)>=convert(date,getdate()) )
ORDER BY PackageName
END
ELSE IF ISNULL(@CONDITION,'')='SPACE'
BEGIN
...
/*
select session_id,login_name,login_time,last_request_start_time,last_request_end_time,
status,cpu_time	memory_usage,total_scheduled_time,total_elapsed_time
,reads,writes,logical_reads,row_count
 from sys.dm_exec_sessions where session_id >51
 USP_REQ_BRNET_JOB_MONITOR 'DSS_Credit_Due_Vs_Col','2022-02-11'
*/